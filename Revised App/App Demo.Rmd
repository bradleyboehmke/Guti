...
# renderValueBox({
# 
#   valueBox(myvalue(),caption = "Block in requested block size", icon = "podcast")
# 
# })
...
# text(scores()[,3],scores()[,5],labels = 1:ncol(reduced_sv()),col = "blue")
  ...
  plot1 = ggplot(scores(),aes(x=FS1,y=FS2,label= 1:nrow(reduced_sv()))) + geom_text()
  plot2 = ggplot(scores(),aes(x=FS3,y=FS4,label= 1:nrow(reduced_sv()))) + geom_text()
  plot3 = ggplot(scores(),aes(x=FS5,y=FS6,label= 1:nrow(reduced_sv()))) + geom_text()
  plot4 = ggplot(scores(),aes(x=FS3,y=FS5,label= 1:nrow(reduced_sv()))) + geom_text()
...
top_cols = reactive({
  top_cols = BD_block()$bd_index[1:hist_range()]
})
...
renderPlot({
par(mfrow=c(2,2))
barplot(t(reduced_sv()[invest_range(),top_cols()]),names.arg = invest_range(), main = "Outlier Block Histogram",xlab = "Block #",ylab = "Count within each block",beside = TRUE,col = rainbow(hist_range()),cex.names = 0.7,las = 1,legend = colnames(reduced_sv())[top_cols()])

barplot(t(reduced_sv()[invest_range(),1:5]),names.arg = invest_range(), main = "Outlier Block Histogram",xlab = "Block #",ylab = "Count within each block",beside = TRUE,col = rainbow(hist_range()),cex.names = 0.7,las = 1,legend = colnames(reduced_sv())[1:5])

barplot(t(reduced_sv()[invest_range(),5:10]),names.arg = invest_range(), main = "Outlier Block Histogram",xlab = "Block #",ylab = "Count within each block",beside = TRUE,col = rainbow(hist_range()),cex.names = 0.7,las = 1,legend = colnames(reduced_sv())[5:10])

barplot(t(reduced_sv()[invest_range(),17:21]),names.arg = invest_range(), main = "Outlier Block Histogram",xlab = "Block #",ylab = "Count within each block",beside = TRUE,col = rainbow(hist_range()),cex.names = 0.7,las = 1,legend = colnames(reduced_sv())[17:21])
})
